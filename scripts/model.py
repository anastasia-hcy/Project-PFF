...
def SSM(nTimes, ndims, n_sparse=None, model=None, A=None, B=None, V=None, W=None, mu0=None, Sigma0=None, muy=None):
    ...
    # if model != "sensor" and V is None:
    #     V           = tf.eye(ndims, dtype=tf.float64) 
    # if model == "sensor" and V is None:
    #     V           = tf.constant([[10.0, 0.0],[0.0, 2.0]], dtype=tf.float64) 
    # if model != "sensor" and W is None:
    #     W           = tf.eye(n_sparse, dtype=tf.float64) 
    # if model == "sensor" and W is None:
    #     W           = tf.constant([[0.04, 0.0],[0.0, 0.04]], dtype=tf.float64) 
