...
# def LGSSM(nTimes, ndims, n_sparse=None, A=None, B=None, V=None, W=None, mu0=None, Sigma0=None):
#     """
...
#     B : tf.Tensor of float64 with shape (n_sparse,ndims), optional. The output matrix.Defaults to matrix of ones if not provided.
...
#     Y : tf.Variable of float64 with dimension (nTimes,n_sparse). The measurements generated by the linear Gaussian SSM.
#     """
...
#     y0              = norm_rvs(ndims, tf.linalg.matvec(B, x0), W)
...
#         yi          = norm_rvs(n_sparse, tf.linalg.matvec(B, xi), W)      
#         X[i,:].assign(xi)
#         Y[i,:].assign(yi)
...
def SSM(nTimes, ndims, n_sparse=None, model=None, A=None, B=None, V=None, W=None, mu0=None, Sigma0=None, muy=None):
    ...
    ndims : int32. Dimension of state space. 
    n_sparse : int32, optional. Dimension of measurements in case of observation sparsity. Defaults to ndims if not provided.
    model: string, optional. The name of the measurement model. Defaults to stochastic volatility "SV" if not provided.
    ...
    muy : tf.Tensor of float64 with shape (n_sparse,), optioanl. The expectation of the measurements. Defaults to zeros if not provided.
...
def UKF_Predict_additive(model, ndims, xpred, Ppred, w0m, w0c, wi, Lamb, B, W, U):
    """Compute and return the estimated means, covariance and cross-covariance of the transformed variables with additive noises."""
    Xpred_sp    = SigmaPoints(ndims, xpred, Ppred, Lamb)
    Y_sp        = UKF_Propagate(model, ndims, Xpred_sp, B) 
    y_pred      = UKF_Predict_mean(w0m, wi, Y_sp)
    W_pred      = UKF_Predict_cov(ndims, w0c, wi, Y_sp, y_pred, U, Cov=W)
    C_pred      = UKF_Predict_crosscov(ndims, w0c, wi, Xpred_sp, xpred, Y_sp, y_pred, U) 
    return Xpred_sp, Y_sp, y_pred, W_pred, C_pred

def UKF_Predict_augmented(model, ndims, xpred, xpred0, Ppred0, w0m, w0c, wi, Lamb, B, W, U):
    """Compute and return the estimated means, covariance and cross-covariance of the transformed variables with non-additive noises."""
    Xpred_sp    = SigmaPoints(ndims*2, xpred0, Ppred0, Lamb)
    Y_sp        = UKF_Propagate(model, ndims, Xpred_sp, B) 
    y_pred      = UKF_Predict_mean(w0m, wi, Y_sp)
    W_pred      = UKF_Predict_cov(ndims*2, w0c, wi, Y_sp, y_pred, U, Cov=W)
    C_pred      = UKF_Predict_crosscov(ndims*2, w0c, wi, Xpred_sp[:,:ndims], xpred, Y_sp, y_pred, U) 
    return Xpred_sp, Y_sp, y_pred, W_pred, C_pred
...
def UnscentedKalmanFilter(y, model=None, A=None, B=None, V=None, W=None, mu0=None, Sigma0=None):
    ...
    for i in range(nTimes):
        ...
        if model == "SV":
            ...
            Xpred_SP, Y_SP, y_pred, W_pred, C_pred = UKF_Predict_augmented(model, ndims, x_pred, x_pred0, P_pred0, weight0_m, weight0_c, weighti, L, B, W, u)

        if model == "LG": 
            Xpred_SP, Y_SP, y_pred, W_pred, C_pred = UKF_Predict_additive(model, ndims, x_pred, P_pred, weight0_m, weight0_c, weighti, L, B, W, u)
...
def EDH(y, model=None, A=None, B=None, V=None, W=None, N=None, mu0=None, Sigma0=None, muy=None, method=None, stepsize=None):
    """
    ...
    for i in range(nTimes): 
        ...
        if method == "UKF":        
            ...
            # ysp                         = UKF_Propagate(model, ndims, xsp, B)
            ...
            if model == "SV" :
                ...
                xsp, ysp, y_pred, R, Rcross  = UKF_Predict_augmented(model, ndims, m_pred, x_pred0, P_pred0, weight0_m, weight0_c, weighti, L, B, W, u)
            if model == "LG": 
                xsp, ysp, y_pred, R, Rcross  = UKF_Predict_additive(model, ndims, m_pred, P_pred, weight0_m, weight0_c, weighti, L, B, W, u)
...
def LEDH_linearize_UKF(N, n, model, I1, xprev, Pprev, A, B, V, W, wm, wc, wi, L, U):
    ...
    for i in range(N): 
        ...
        if model == "SV" :
            x_pred0[:n].assign(mi_pred)
            P_pred0[0:n,0:n].assign(Pi_pred)
            Xpred_sp, Y_sp, yi_pred, W_pred, Rcross = UKF_Predict_augmented(model, n, mi_pred, x_pred0, P_pred0, wm, wc, wi, L, B, W, U)
        if model == "LG": 
            Xpred_sp, Y_sp, yi_pred, W_pred, Rcross = UKF_Predict_additive(model, n, mi_pred, Pi_pred, wm, wc, wi, L, B, W, U)
...
def LEDH(y, model=None, A=None, B=None, V=None, W=None, N=None, mu0=None, Sigma0=None, muy=None, method=None, stepsize=None):
    """
...
def KernelPFF(y, Nx, model=None, A=None, B=None, V=None, W=None, N=None, mu0=None, Sigma0=None, muy=None, method=None, stepsize=None):
    """
    ...
    if model != "SV" and Sigma0 is None : 
        Sigma0      = tf.eye(Nx, dtype=tf.float64)
    muy             = tf.zeros((ndims,), dtype=tf.float64) if muy is None else muy
