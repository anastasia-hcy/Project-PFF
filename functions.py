...
def measurements(model, n, m, B, x, W, U=None):
    ...
    if model == "SV" :
        ...
        return SV_transform(n, m, B, x, W, U)
...
def SSM(nTimes, ndims, n_sparse=None, model=None, A=None, B=None, V=None, W=None, mu0=None, Sigma0=None, muy=None):
    ...
    if model == "SV" and Sigma0 is None :
        Sigma0      = (V @ V) @ tf.linalg.inv(tf.eye(ndims, dtype=tf.float64) - A @ A)  
    if model != "SV" and Sigma0 is None: 
        Sigma0      = tf.eye(ndims, dtype=tf.float64)
...
def KernelPFF(y, Nx, model=None, A=None, B=None, V=None, W=None, N=None, Nstep=None, mu0=None, Sigma0=None, muy=None, method=None, stepsize=None):
    ...
    if model == "SV" and Sigma0 is None :
        Sigma0      = (V @ V) @ tf.linalg.inv(tf.eye(Nx, dtype=tf.float64) - A @ A)  
    if model != "SV" and Sigma0 is None : 
        Sigma0      = V
    muy             = tf.zeros((ndims,), dtype=tf.float64) if muy is None else muy
