...
from scripts import LocalExactDH
...
def background_ledh_ekf_sde(y, N, path):
    
    start_cpu_time  = time.process_time()
    initial_rss     = psutil.Process(os.getpid()).memory_info().rss

    X1_LEDH_EKF_SDE, ess1_LEDH_EKF_SDE, weights1_LEDH_EKF_SDE, Jx1_LEDH_EKF_SDE, Jw1_LEDH_EKF_SDE = LEDH.run(y=y, N=N, stochastic=True)
    ...
    with open(path+"res_LEDH_EKF_SDE_LG.pkl", 'wb') as file:
        pkl.dump({"res": X1_LEDH_EKF_SDE, 
                  "ess": ess1_LEDH_EKF_SDE,
                  "weights": weights1_LEDH_EKF_SDE,
                  "Jx": Jx1_LEDH_EKF_SDE, 
                  "Jw": Jw1_LEDH_EKF_SDE,
                  "cpu": [cpu_time_taken, memory_increase_mib]}, file)

def background_ledh_ukf_sde(y, N, path):
    
    start_cpu_time  = time.process_time()
    initial_rss     = psutil.Process(os.getpid()).memory_info().rss

    X1_LEDH_SDE, ess1_LEDH_SDE, weights1_LEDH_SDE, Jx1_LEDH_SDE, Jw1_LEDH_SDE = LEDH.run(y=y, N=N, method='UKF', stochastic=True)
    ...
    with open(path+"res_LEDH_SDE_LG.pkl", 'wb') as file:
        pkl.dump({"res": X1_LEDH_SDE, 
                  "ess": ess1_LEDH_SDE,
                  "weights": weights1_LEDH_SDE,
                  "Jx": Jx1_LEDH_SDE, 
                  "Jw": Jw1_LEDH_SDE,
                  "cpu": [cpu_time_taken, memory_increase_mib]}, file)
...
if __name__ == '__main__':
    ...
    p2 = multiprocessing.Process(target=background_ledh_ukf_sde, args=(Y1, Np, pathres,))
    p2.start()
    
    p3 = multiprocessing.Process(target=background_ledh_ekf_sde, args=(Y1, Np, pathres,))
    p3.start()
